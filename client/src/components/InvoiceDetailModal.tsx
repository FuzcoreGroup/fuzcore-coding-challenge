...
export function InvoiceDetailModal({ isOpen, onClose, invoice }: InvoiceDetailModalProps) {
  const { updateInvoiceStatus, loading } = useInvoices();
  ...
  const handleExportPDF = () => {
    ...
    try {
      ...
      addNotification({
        ...
        message: `Invoice #${invoice.id} has been exported as PDF.`,
      });
    } catch (error) {
  ...
  const canMoveToStatus = (targetStatus: InvoiceStatus) => {
        ...
        <div className="space-y-6">
              ...
              <table className="w-full">
                ...
                <tbody>
                  {invoice.lineItems.map((item) => (
                    <tr key={item.id} className="border-t border-[#e6eff5]">
                      <td className="py-3 px-4 text-[14px] text-[#343c6a]">{item.name}</td>
                      <td className="text-center py-3 px-4 text-[14px] text-[#718ebf]">{item.quantity}</td>
                      <td className="text-right py-3 px-4 text-[14px] text-[#718ebf]">
                        ${item.price.toFixed(2)}
                      </td>
              ...
              <div className="flex gap-3">
                {invoice.status === 'draft' && (
                  <button
                    ...
                    disabled={loading}
                    className="flex-1 h-[45px] bg-[#ffbb38] text-white font-medium text-[14px] rounded-[10px] hover:bg-[#eda10d] transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                ...
                {(invoice.status === 'draft' || invoice.status === 'sent') && (
                  <button
                    ...
                    disabled={loading}
                    className="flex-1 h-[45px] bg-[#16dbcc] text-white font-medium text-[14px] rounded-[10px] hover:bg-[#12b8a3] transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                  >
          ...
          <div className="flex gap-3 pt-4">
            <button
              ...
              className="flex-1 h-[50px] bg-[#2d60ff] text-white font-medium text-[16px] rounded-[15px] hover:bg-[#1814f3] transition-colors flex items-center justify-center gap-2"
            >
              <svg width="20" height="20" viewBox="0 0 20 20" fill="none">
